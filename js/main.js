...
Promise.all([ // load multiple files
	d3.csv('data/globalterrorism_year.csv', d=>{
	...
	d3.csv('data/globalterrorism_category.csv', d=>{
		Object.keys(d).forEach(key=>{
			if (key != "Year") {
				d[key] = parseFloat(d[key]) * 1.0;
			} else if(key == "Year") {
		...
		// 	Fake_Weapons: +d.Fake_Weapons,
		// 	Firearms: +d.Firearms,
		// 	Incendiary: +d.Incendiary,
		// 	Melee: +d.Melee,
		// 	Other: +d.Other,
		// 	Radiological: +d.Radiological,
		// 	Sabotage_Equipment: +d.Sabotage_Equipment,
		// 	Unknown: +d.Unknown
		// 	}
	})
]).then(data=>{
